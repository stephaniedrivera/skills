---
name: docs-content-strategy
description: "Evaluate a feature summary from project-brain-synthesis and decide whether to write a new Help Center article, revise existing articles, or both — then hand that decision to docs-writer and/or docs-refresh. Use this skill whenever a feature summary exists and the next question is what to do with it documentationally. Trigger on 'what do we need to document', 'do we need a new article', 'what needs updating', 'coverage decision', or any time project-brain-synthesis has just run and the writing step hasn't started yet. Do NOT use this for general information architecture, content audits, or restructuring work unrelated to a specific feature — that is broader IA strategy work handled separately."
---

# Help Center content strategy

Takes a feature summary from `project-brain-synthesis` and decides the documentation
approach. Outputs a decision packet that goes directly to `docs-writer`, `docs-refresh`,
or both. This skill decides; it does not write.

## What to do

1. **Check ship status.** Before searching the repo, confirm the feature is shipped
   and user-facing. Use the Linear project status and the feature summary's ship
   status field:
   - **Shipped / completed** → proceed normally.
   - **In progress / in experiment / backlog, AND the caller has explicitly flagged
     this as a pre-launch staging run** (e.g., invoked from the Week 1 / "draft"
     stage of the weekly doc pipeline, or the user says they want to draft ahead of
     a known ship date) → proceed, but mark the decision packet **"Provisional —
     not yet shipped as of [date]."** Say so plainly in the handoff so docs-writer/
     docs-refresh and solo-verify know they're drafting against a moving target —
     downstream, expect more "Unverifiable" claims from solo-verify than a
     post-ship run would produce, since Ask Solo's index may not reflect
     unshipped code yet. That's expected, not an error, at this stage.
   - **In progress / in experiment / backlog, with no such flag** → stop. Don't
     make a coverage decision for an unshipped feature on your own initiative.
     Report the status back and ask the user whether to proceed anyway or wait.
   - **Status unclear** → flag it and ask before proceeding.

2. **Read the feature summary.** The input is a structured summary from
   `project-brain-synthesis` — feature name, new capabilities, affected UI areas,
   related features, caveats. If no summary is present, stop and ask for one before
   proceeding.

3. **Search the live repo** for existing articles that touch this feature. Look
   across the `help-center/` directory and `docs.json` using the feature name,
   related UI surfaces, and workflow terms from the summary. Cast a wide net —
   better to review and discard a false match than miss a page that needs updating.

4. **Classify each match:**
   - **Needs content update** — instructions or UI descriptions are now wrong or
     incomplete given the new feature.
   - **Needs crosslink only** — content is still accurate, but should reference the
     new feature.
   - **No relation** — discard.

5. **Decide:**
   - **New article** — no existing page owns this job, or absorbing it would force
     an existing page to cover two unrelated jobs.
   - **Revise existing** — one or more matches need a content update and can absorb
     the new feature cleanly.
   - **Both** — the feature needs a new standalone article AND touches existing
     pages that need updating or crosslinking. This is the common case for a
     hub feature or a significant capability addition.

6. **If the fit is ambiguous** — two equally plausible existing homes, or genuinely
   unclear whether this warrants a new article — flag it. Don't pick silently.
   Surface the options and the tradeoff so the human can decide.

7. **Output the decision packet** (see format below) and hand it to:
   - `docs-writer` if the decision includes a new article
   - `docs-refresh` if the decision includes revising existing articles
   - Both, in that order, if the decision is "both"

## Decision packet format

```
## Coverage decision: [Feature name]
Ship status: shipped / provisional (pre-launch) / unclear
Decision: new article / revise existing / both
Target article(s): [title(s) + repo path — existing or proposed]
Section/Category: [nav placement per docs.json]
Reasoning: [1-2 sentences tied to the evidence above]
Crosslinks: [inbound / outbound, if any]
Open items: [gaps, ambiguities, or none]
```

The packet is the handoff — everything docs-writer and docs-refresh need is in it.
Don't proceed to drafting yourself.
